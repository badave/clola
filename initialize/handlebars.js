...
// Currency math operators
hbs.registerHelper('currency', function(lvalue, operator, rvalue, options) {
  var operators = {
    '+':      function(l,r) { return l + r; },
    '-':       function(l,r) { return l - r; },
    '*':       function(l,r) { return l * r; },
  ...
  if (operator && (rvalue !== undefined)) {
    if (!operators[operator]) {
      throw new Error("Handlerbars Helper 'currency' doesn't know the operator " + operator);
    }
  ...
  var formattedAmount =  accounting.formatMoney(amountInCents / 100);
  return formattedAmount;
});

// Formats currency ($X.XX)
hbs.registerHelper('formatCurrency', function(lvalue, operator, rvalue, options) {
  var operators = {
    '+':      function(l,r) { return l + r; },
    '-':       function(l,r) { return l - r; },
    '*':       function(l,r) { return l * r; },
...
// Formats currency without '$' sign (X.XX)
hbs.registerHelper('formatNumber', function(amountInCents, options) {
  return accounting.formatMoney(amountInCents / 100, '');
});
...
hbs.registerHelper('formatDateSeconds', function(value, options) {
  var ms = value * 1000;
  return moment(ms).format('MM/DD/YYYY');
});
...
hbs.registerHelper('formatGmailTimeLong', function(value, options) {
  var ms = value;
...
hbs.registerHelper('formatStatus', function(value, options) {
  var status = value;
  if (status === 'charged_deposit') {
    status = 'paid';
  } else if (status === 'charged_balance') {
    status = 'paid';
  } else if (status === 'refunded_deposit') {
    status = 'refunded';
  } else if (status === 'refunded_balance') {
    status = 'refunded';
  }
  return status;
});
